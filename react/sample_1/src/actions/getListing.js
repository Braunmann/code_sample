...
`;
export function getListingData(listId, listType) {
    ...
    try {
      ...
      if (data && data.UserListing) {
        ...
        data.UserListing.settingsData.map((item, value) => {
         if(item.listsettings.settingsType.typeName == 'buildingSize'){
            settingFieldsData['ageOfDriver'] = item.settingsId;
          } else if(item.listsettings.settingsType.typeName == 'roomType'){
            settingFieldsData['model'] = item.settingsId;
          } else{
        ...
        if (!listType){
          listType = data.UserListing.listType;
        }
        let securityDeposit;
        if (data.UserListing && data.UserListing.residenceType) {
          securityDeposit = data.UserListing.residenceType
        }
        
        settingFieldsData = Object.assign({}, settingFieldsData, { bedTypes, listType, securityDeposit });
