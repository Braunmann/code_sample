...
class PaymentForm extends Component {
  ...
  renderGuests(personCapacity) {
    ...
    for (let i = 1; i <= personCapacity; i++) {
      rows.push(<option key={i} value={i}>{i} {i > 1 ? 'seats' : 'seat'}</option>);
    }
    return rows;
  }
  ...
  render() {
    ...
    return (
      <div className={cx(s.bookItPanel, 'spaceTop4')}>
        <form onSubmit={handleSubmit(submit)}>
          <Row>
            <Col md={10} className={cx(s.textLeft)}>
              ...
              <div className={cx(s.bookItDetails, 'spaceTop2', 'space4', 'hidden')}>
